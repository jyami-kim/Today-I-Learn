...
- read : fdopen 함수가 쓰기위해 여는 파일은 자를 수 없다.
- append : 각각의 쓰기 연산이 파일의 끝에서 일어난다. 
- read and write : 출력 직후 입력이 불가능하다. 그 사이에 fflush 등을 호출해야한다. / 입력 직후 출력이 불가능하다. 그 사이에 fseek 등을 호출해야한다.
